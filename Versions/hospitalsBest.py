...
class Space():
    ...
    def hill_climb(self, maximum=None, image_prefix=None, log=False):
        ...
        for i in range(self.num_hospitals):
            self.hospitals.append(random.choice(list(self.available_spaces())))
        if log:
        ...
        while maximum is None or count < maximum:
            ...
            for hospital in self.hospitals:
                ...
                for replacement in self.get_neighbors(*hospital):
                    ...
                    index = self.hospitals.index(hospital)
                    neighbor.remove(hospital)
                    neighbor.insert(index, replacement)
    ...
    def generate_heat_map(self, hospitals):
        """
        ...
        for i in range(self.height):
            for j in range(self.width):
                # Compute the Manhattan distance to each hospital, and find the minimum distance
                min_distance = min(abs(h[0] - i) + abs(h[1] - j) for h in hospitals)
                # Sum this minimum distance for all houses
                heat_map[i, j] = sum(min_distance for house in self.houses)
    ...
    def draw_path(self):
        ...
        #Mark the initial hospital location
        for h in self.path[0]:
            plt.scatter(h[1], h[0], c='green', s=100, label="Hospital", marker="^")

        # Mark the final hospital location
        for h in self.path[-1]:
            plt.scatter(h[1], h[0], c='green', s=100, label="Hospital", marker="*")
